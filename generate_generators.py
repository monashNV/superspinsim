...
if __name__ == "__main__":
    # Generate operators
    _operators = {}
    for label, operator_generator in _operator_generators.items():
        _operators[label] = operator_generator()

    # Generate superoperators
    _superoperators = {}
    for label, operator in _operators.items():
        if "spin" in label:
            _superoperators[label] = _generate_von_neumann(operator)
        else:
            _superoperators[label] = _generate_jump(operator)
    ...
    for label in [
        ...
        _superoperators_use[label] = _superoperators[label]

    _superoperators_use["decay_pm_s"] = _superoperators["decay_p_s"] \
        + _superoperators["decay_m_s"]

    _superoperators_use["decay_conserve"] = _superoperators["decay_p_p"] \
        + _superoperators["decay_z_z"] + _superoperators["decay_m_m"]

    _superoperators_use["decay_nonconserve"] = _superoperators["decay_p_z"] \
        + 0.5*_superoperators["decay_z_p"] + 0.5*_superoperators["decay_m_m"] \
        + _superoperators["decay_m_z"]

    _superoperators_use["raise_conserve"] = _superoperators["raise_p_p"] \
        + _superoperators["raise_z_z"] + _superoperators["raise_m_m"]

    _superoperators_use["raise_nonconserve"] = _superoperators["raise_p_z"] \
        + 0.5*_superoperators["raise_z_p"] + 0.5*_superoperators["raise_m_m"] \
        + _superoperators["raise_m_z"]
    ...
    for operator_index, (label, operator) in enumerate(
        ...
        plt.subplot(
            _number_of_rows,
            math.ceil(len(_operators)/_number_of_rows),
            1 + operator_index
        )
    ...
    for operator_index, (label, superoperator) in enumerate(
            _superoperators.items()):
        plt.subplot(
            _number_of_rows,
            math.ceil(len(_superoperators)/_number_of_rows),
            1 + operator_index
        )
    ...
    plt.figure()
    for operator_index, (label, superoperator) in enumerate(
            _superoperators_use.items()):
        plt.subplot(
            _number_of_rows,
            math.ceil(len(_superoperators_use)/_number_of_rows),
            1 + operator_index
        )
        plt.imshow(_colour_complex_matrix(superoperator))
        plt.axis("off")
